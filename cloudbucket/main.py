import os, re, gzip, json, urllib.parse, urllib.request, traceback, datetime, calendar, hashlib, sys
...
def load_config_field_value(gcs_file_path):
    for field_name in config_types:
        config_type = config_types[field_name]
        if '@filepath' in config_type:
            if ':' in config_type and int(config_type[10:]) < gcs_file_path.count(os.sep):
                config_fields[field_name] = gcs_file_path.split(os.sep)[int(config_type[10:])]
        ...
        if line:
            try:
                ...
                if matcher:
                    ...
                    if is_date_present is None and parsed_lines:
...
def send_logs_to_s247(gzipped_parsed_lines, log_size):
    header_obj = {'X-DeviceKey': logtype_config['apiKey'], 'X-LogType': logtype_config['logType'],
                  'X-StreamMode' :1, 'Log-Size': log_size, 'Content-Type' : 'application/json', 'Content-Encoding' : 'gzip', 'User-Agent' : 'GCP-CloudFunction'
    }
...
def process_gcs_event(cloud_event):
    """Background Cloud Function to be triggered by Cloud Storage.
       This function is triggered when a file is uploaded to the specified GCS bucket.
    ...
    try:
        ...
        print(f"File content downloaded for file: {file_name}")
        
        # Process content based on file type
        if file_name.endswith('.gz') or blob.content_type == 'application/x-gzip':
            lines_read = gzip.decompress(content)
        else:
            lines_read = content
            
        # Load configuration field values if needed
        if config_types:
            load_config_field_value(file_name)
        ...
        if 'jsonPath' in logtype_config:
            ...
            parsed_lines, log_size = json_log_parser(records_obj, bucket_name)
        elif s247_ml_regex:
        ...
        return {
                'statusCode': 200,
                'body': json.dumps('No logs to upload')
                }
            
    except Exception as e:
        error_message = f"Error processing file {file_name} from bucket {bucket_name}: {str(e)}"
        print(error_message)
        traceback.print_exc()
